...
The immutable intake pack is the durable local processed identity; generated receipts are disposable and reconstructible. Repeated pulls compare the selected live snapshot to that binding without overwriting it. Body/title/author/URL/`updated_at` changes are drift, even when request bytes still match. Deleted/inaccessible processed Issues require reconciliation. Timestamp-only GitHub updates can therefore require review; unrelated drift is reported without globally disabling intake.
